...
export const config = {
  ...
  // Two models to keep cost down:
  // - searchModel handles the token-HEAVY search+fetch step → cheapest model.
  // - rateModel does the low-token scoring where judgment matters → mid model.
  ...
  rateModel:
    process.env.RATE_MODEL || process.env.CLAUDE_MODEL || "claude-haiku-4-5",
  ...
  searchCount: Number(process.env.SEARCH_COUNT || 8), // results per query
  ...
  // Matches the "posted within the last 7 days" preference in your profile.
  maxPostedDays: Number(process.env.MAX_POSTED_DAYS || 7),
